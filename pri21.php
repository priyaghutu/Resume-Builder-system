...
$db = mysqli_select_db($conn,DB_NAME) or die("Failed to connect to MySQL: " . mysqli_error());//selecting database from the established connection
 $sql2="SELECT * FROM education1 WHERE username = '$x'"; //selecting all values from database table where username matches the entered username
...
if($row1 = mysqli_fetch_array($query2))//reterieving an associative array from the fetched query and further checking whether it is empty or not
{
    ...
    $l1=$row1['qualification'];
     $m1=$row1['organisation'];
      $n1=$row1['city2'];
       $o1=$row1['country2'];
       $chk11=$row1['chk2'];
       $ch4=$row1['ch4'];
...
if(empty($e1)||empty($f1)||empty($g1))
{
 ...
 if((!empty($h1)&&!empty($i1)&&!empty($j1))||!empty($chk11))
  ...
  $db = mysqli_select_db($conn,DB_NAME) or die("Failed to connect to MySQL: " . mysqli_error());//selecting database from the established connection
   ...
   $query = "update education1 set from2 ='$arr', to2='$arr1',chk2='$chk11',qualification='$l1',organisation='$m1',city2='$n1',country2='$o1',ch4='$ch4',st='$p1',pc='$q1',e1='$e1',f1='$f1',g1='$g1',h1='$h1',i1='$i1',j1='$j1',website='$w' where username='$x'";
...
<div class="center" id="mydiv">
  <form action="pri21.php" method="post">
