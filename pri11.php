...
if(empty($e1)||empty($f1)||empty($g1))
{
 ...
 if((!empty($h1)&&!empty($i1)&&!empty($j1))||!empty($chk11))
  ...
  $db = mysqli_select_db($conn,DB_NAME) or die("Failed to connect to MySQL: " . mysqli_error());//selecting database from the established connection
    ...
    $query = "insert into education1 values('$arr','$arr1','$chk11','$l1','$m1','$n1','$o1','$ch4','$p1','$q1','$e1','$f1','$g1','$h1','$i1','$j1','$x','$w')";//insert in the same order as the database
...
<input type="search" name="website" placeholder="wwww.abc.com" autocomplete=off style="width:400px;" value="<?php if(!empty($w))echo $w; else ;?>" ></div>
